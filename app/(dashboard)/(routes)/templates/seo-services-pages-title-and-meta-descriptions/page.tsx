...
const formSchema = z.object({
  ...
  serviceName: z.string().min(4).max(100),
...
const Page = () => {
  ...
  const { mutate, isLoading } = useMutation({
    ...
    mutationFn: async ({
      ...
      const prompt = `Write an SEO meta title and meta description of the service page with service name ${serviceName}, for the company named ${companyName}. The description of the product is ${productDescription}. Write an SEO meta title and meta description using the keywords ${keywords}. The SEO meta title and meta description should be compelling so that the viewer is tempted to interact. Remember to only write an SEO meta title and meta description for the given input service page. Note:- Do not write other than the SEO meta title and meta description. Important point:- The meta-title has a word limit of 10 words so do not write the meta-title more than 10 words and the meta-description has a word limit of 20 words so do not write the meta-description more than 20 words.
      Response format:-
      Title:- meta title
      ...
      await axios.post("/api/history", {
        prompt: `company name : ${companyName} | service name : ${serviceName} | keywords : ${keywords} | product description : ${productDescription}`,
        content: dataArray.join("|"),

        type: HistoryType.SEO_SERVICES_PAGES_TITLE_AND_META_DESCRIPTIONS,
      });
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="w-full">
                  ...
                  <FormControl>
                    <Input placeholder="Name of your service" {...field} />
                  </FormControl>
                  <FormDescription className="text-right">
                    {form.watch("serviceName").length}/100
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="w-full">
                  ...
                  <FormControl>
                    <Input
                      placeholder="Keywords related to your company"
                      {...field}
                    />
                  </FormControl>
                  <FormDescription className="text-right">
