...
const Layout: FC<LayoutProps> = async ({ children }) => {
  const history = await getHistory("SENTENCE_ELABORATOR");
  return (
    <div className="relative h-[calc(100vh-4.5rem)] overflow-y-auto flex md:divide-x">
      <div className="flex grow overflow-y-auto flex-col">
        <div className="flex justify-between items-start pr-6">
          <Heading
            title="SENTENCE ELABORATOR"
            description="Transform a brief sentence or a small group of words into a more extensive sentence that is original, captivating, and stimulating."
            icon={Zap}
            iconColor="text-gray-500 dark:text-gray-300"
            bgColor="bg-gray-100 dark:bg-gray-800"
          />
          <HistorySidebarMobile
            generateNewEndpoint="/templates/sentence-elaborator"
            history={history}
          />
        </div>
