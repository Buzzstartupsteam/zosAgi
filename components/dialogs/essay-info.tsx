...
const stepList = [
  "Please provide the topic for the essay you would like to create.",
  `Pick the content's tone of voice from the options provided.`,
  `You can access all the previous generations and generate new essay by clicking on 'Generate New'.`,
];
...
const EssayInfo = () => {
  ...
  const setDone = () => {
    setIsNew(false);
    if (typeof window !== "undefined" && window.localStorage) {
      localStorage.setItem("essay", "true");
    }
  };
        ...
        <div
          className={cn(
            "p-4 rounded-xl border border-black dark:border-white absolute",
            {
              ...
              "h-12 w-56 left-[1.65rem] top-[12.25rem]": activeStep == 2,
